...
- Источник (обязательно): если в блоке «Авторитетные источники» есть URL — используй первый. Если блок пуст — самостоятельно определи наиболее авторитетный домен по теме поста (официальный сайт компании или крупное СМИ: tass.ru, roscosmos.ru, techcrunch.com, reuters.com и т.п.) и укажи только домен без пути. Ссылки на Telegram-каналы (t.me) запрещены
...
export async function generatePostFromSources(
  ...
  const sourceBlock =
    foundUrls.length > 0
      ...
      : "Авторитетные источники: URL не найден — определи подходящий домен самостоятельно по теме поста";
